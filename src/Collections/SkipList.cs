...
namespace XLR8.Collections
{
        ...
        where TK : IComparable<TK>
    {
        ...
        public SkipList(Comparison<TK> comparer)
        {
            _head = new Node();
            _head.Next = new Node[16];
            _count = 0;
        ...
        /// <returns></returns>
        private Node Remove(Node head, TK key, int level, out bool found)
        {
            found = false;
        ...
        public bool Remove(TK key)
        {
            ...
            {
                curr = Remove(curr, key, ii, out found);
            }
        ...
        public IDictionary<TK, TV> Between(TK startValue, bool isStartInclusive, TK endValue, bool isEndInclusive)
        {
            if (_comparer != null)
            {
                if (_comparer.Invoke(startValue, endValue) > 0)
                {
        ...
        public SkipList<TK, TV> Invert()
        {
            ...
            var inverted = new Comparison<TK>((a, b) => -comparer.Invoke(a, b));
        ...
        internal class Submap : IDictionary<TK, TV>
        {
            ...
            internal Submap(SkipList<TK, TV> subDictionary, Bound lower, Bound upper)
            {
                ...
                {
                    var comparer = _subDictionary._comparer ?? DefaultComparer;
                    _lowerTest = MakeLowerTest(comparer);
                    _upperTest = MakeUpperTest(comparer);
                }
            }

            private Func<TK, bool> MakeUpperTest(Comparison<TK> keyComparer)
            {
                if (_upper.HasValue)
                {
                    if (_upper.IsInclusive)
                    {
                        return k1 => keyComparer.Invoke(k1, _upper.Key) <= 0;
                    }
                    else
                    {
                        return k1 => keyComparer.Invoke(k1, _upper.Key) < 0;
            ...
            private Func<TK, bool> MakeLowerTest(Comparison<TK> keyComparer)
            {
                if (_lower.HasValue)
                {
                    if (_lower.IsInclusive)
                    {
                        return k1 => keyComparer.Invoke(k1, _lower.Key) >= 0;
                    }
                    else
                    {
                        return k1 => keyComparer.Invoke(k1, _lower.Key) > 0;
